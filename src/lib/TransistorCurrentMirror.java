...
public class TransistorCurrentMirror extends BuildingBlocks{
	
	VoltageRef1 vr = new VoltageRef1();
	...
	public <V, E> Graph<Node, Connection> createBuildingBlock() {
		...
		c1.addConnection("1_"+ vr.getM1().getSourceID(), "2_"+scm.getM1().getDrainID());
		c1.addNodes(vr.getM1());
		c1.addNodes(scm.getM1());
		
		g.addEdge(c1, c1.getNodes(), EdgeType.UNDIRECTED);
		
		c2.addConnection("1_"+ vr.getM2().getSourceID(), "2_"+ scm.getM2().getDrainID());
		c2.addNodes(vr.getM2());
	...
	public boolean getSizingRule(Mosfet m1) {
		...
		list.add("wvrI(1) = wvrI(2)");
		list.add("wcml(1) = wcml(2)");
		list.add("vdsvrI(2) - vdscml(2) | <= Vdsmax(4TCM)");
	...
	public int getPriority() {
		return ListBB.getPriorityMap().get("Transistor Current Mirror");
	}
